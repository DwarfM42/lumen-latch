...
依存するNumPy、Matplotlib、pytestのversionは[requirements.txt](requirements.txt)に固定しています。手順はWindows 10/11、Ubuntu、macOSを想定していますが、現時点で実動確認済みの環境は**Windows 10 + Git Bash + Python 3.11.15**のみです。PowerShell、Linux、macOSの手順は用意していますが、各実機での検証はまだ行っていません。

#### Windows PowerShell

```powershell
git clone https://github.com/DwarfM42/lumen-latch.git
cd lumen-latch
...
python simulate_optical_cell.py --output-dir . --seed 20260715 --trials 10000
python system_energy_model.py --output system_energy_budget.csv
python -m pytest tests -q
```

PowerShellで仮想環境の実行が拒否される場合は、現在のprocessだけexecution policyを緩和します。
...
実行後、次の成果物が生成または更新されます。

```text
simulation_results.csv
system_energy_budget.csv
figures/
```

#### 再現性確認

同じseedとtrial数で再実行し、CSVのSHA-256を確認します。

Windows PowerShell：

```powershell
Get-FileHash simulation_results.csv -Algorithm SHA256
Get-FileHash system_energy_budget.csv -Algorithm SHA256
```

macOS：

```bash
shasum -a 256 simulation_results.csv system_energy_budget.csv
```
...
CSVは固定seed・固定依存versionで基準hashとの一致を確認します。PNGはOSのfont/rendering stackによりbyte列が変わる可能性があるため、cross-platformでは枚数と内容を確認し、byte-identicalを必須条件にしません。
